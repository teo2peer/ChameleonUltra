...
#define SEOS_ENCRYPTION_AES         NFC_TAG_SEOS_ENCRYPTION_AES

#define SEOS_HASHING_SHA1           0x06
#define SEOS_HASHING_SHA256         NFC_TAG_SEOS_HASHING_SHA256
...
static uint16_t round_to_next(uint16_t value, uint8_t step) {
    if (value % step == 0) {
        return value;
    } else {
...
static void nfc_tag_seos_activate(uint8_t fsdi, uint8_t cid) {
    if (m_tag_information == NULL) return;
    nfc_tag_14a_4_base_activate(&m_tcl_session_state,
                                &m_tag_information->res_coll.ats,
                                fsdi, cid);
}
...
static void nfc_tag_seos_state_handler(uint8_t *data, uint16_t szBits) {
    if (m_tag_information == NULL || data == NULL ||
            (szBits & 0x07u) != 0u) return;
    ...
    switch (receivedCmd[1]) { // APDU Class Byte
        case 0xA4: {  // SELECT FILE
            ...
            if (receivedCmdLen < 5u) break;
            uint8_t aid_len = receivedCmd[4];
            if (aid_len > receivedCmdLen - 5u) break;
        ...
        case 0xA5: {  // SELECT OID
            ...
            if (received_tlv_len > receivedCmdLen - 5u) break;
            ...
            if (selected_oid) {
                ...
                if (4u + m_tag_information->oid_len +
                        m_tag_information->diversifier_len > reply_len) {
                    NRF_LOG_ERROR("Seos: Select ADF failed: Credential fields are too long.");
                    break;
                }
        ...
        case 0x87: {  // MUTUAL AUTH
            ...
            if (receivedCmdLen < 7u) break;
            uint8_t received_tlv_len = receivedCmd[4];
            if (received_tlv_len > receivedCmdLen - 5u || received_tlv_len < 4u) break;
            uint8_t *received_tlv = &receivedCmd[5];
            ...
            uint8_t outer_len = received_tlv[1];
            if (outer_len > received_tlv_len - 2u || outer_len < 2u) break;
            received_tlv += 2;
            uint8_t inner_available = outer_len;

            if (received_tlv[0] == 0x81) {
                if (received_tlv[1] > inner_available - 2u) break;
                // Request for RND.ICC
                uint8_t tlv_idx = 0;
                ...
                if (received_tlv_len > inner_available - 2u ||
                        received_tlv_len > WORK_BUFFER_SIZE) {
                    NRF_LOG_ERROR("Seos: Mutual auth failed: Recieved cryptogram too long.");
                    break;
                }

                if (received_tlv_len < 40u ||
                        ((received_tlv_len - 8u) % bs) != 0u) {
                ...
                if (!generate_cryptogram(diver_encr_key, false, reply_plain,
                                         reply_plain_len, reply,
                                         m_tag_information->encr_alg)) {
                    NRF_LOG_ERROR("Seos: Mutual auth failed: Failed to create reply cryptogram.");
                    break;
                }
        ...
        case 0xCB: { // GET DATA
            ...
            while (tlv_offset + 2 <= received_tlv_len) {
                ...
                if ((length & 0x80) == 0x80) {
                    ...
                    if (tlv_offset >= received_tlv_len) break;
            ...
            if (tlv_offset != received_tlv_len) {
                NRF_LOG_ERROR("Seos: Get Data failed: Malformed TLV payload.");
                break;
            }

            if (cryptogram != NULL && recvd_cmac != NULL) {
                if (cryptogram_length == 0u || cryptogram_length > WORK_BUFFER_SIZE ||
                        (cryptogram_length % bs) != 0u ||
                        recvd_cmac_length == 0u || recvd_cmac_length > 16u) {
                ...
                uint8_t *request = work_buffer_a;
                if (!decrypt_cryptogram(diver_encr_key, cryptogram,
                                        cryptogram_length, request,
                                        m_tag_information->encr_alg)) {
                    NRF_LOG_ERROR("Seos: Get Data failed: Unable to decrypt request.");
                    break;
                }
                ...
                if (is_put) {
                    ...
                    //5c 02 ff 00
                    if (cryptogram_length < 2u || request[0] != 0x5C) {
                        NRF_LOG_ERROR("Seos: Get Data failed: Invalid request TLV. Expected tag 5C, but got %02X.", request[0]);
                        break;
                    }

                    if (request[1] != m_tag_information->data_tag_len ||
                            (uint16_t)request[1] + 2u > cryptogram_length ||
                            memcmp(request + 2, m_tag_information->data_tag,
                                   m_tag_information->data_tag_len) != 0) {
                        NRF_LOG_ERROR("Seos: Get Data failed: Requested invalid data tag.");
                        break;
                    }

                    uint16_t reply_len = (uint16_t)m_tag_information->data_tag_len +
                                         1u + m_tag_information->data_len;
                    reply_len = round_to_next(reply_len, bs);
                    if (reply_len > WORK_BUFFER_SIZE || reply_len > 0xFFu) {
                    ...
                    if (2u + reply_len + 4u + 2u + recvd_cmac_length > NFC_14A_4_MAX_APDU) {
                    ...
                    m_tcl_session_state.m_resp_buf[tlv_idx++] = 0x85; // Tag: cryptogram
                    m_tcl_session_state.m_resp_buf[tlv_idx++] = (uint8_t)reply_len; // Length
...
respond:
    ...
    if (m_tcl_session_state.m_resp_len > NFC_14A_4_MAX_APDU - 2u) {
        m_tcl_session_state.m_resp_len = 0u;
        apdu_status[0] = 0x6F;
        apdu_status[1] = 0x00;
    }
...
int nfc_tag_seos_data_loadcb(tag_specific_type_t type, tag_data_buffer_t *buffer) {
    ...
    nfc_tag_14a_4_reset_state(&m_tcl_session_state);
    if (type != TAG_TYPE_SEOS || buffer == NULL || buffer->buffer == NULL ||
            buffer->length < info_size) {
        NRF_LOG_ERROR("SEOS loadcb: buffer too small (%d < %d)",
                      buffer == NULL ? 0 : buffer->length, info_size);
        return 0;
    }
    if (buffer->actual_length != 0u && buffer->actual_length != stored_size) {
        NRF_LOG_ERROR("SEOS loadcb: invalid record length (%d != %d)",
                      buffer->actual_length, stored_size);
        return 0;
    }
    nfc_tag_seos_information_t *candidate =
        (nfc_tag_seos_information_t *)buffer->buffer;
    if (!is_valid_uid_size(candidate->res_coll.size) ||
            candidate->oid_len > NFC_TAG_SEOS_OID_MAX ||
            candidate->data_tag_len > NFC_TAG_SEOS_DATA_TAG_MAX ||
            candidate->diversifier_len > NFC_TAG_SEOS_DIVERSIFIER_MAX ||
            candidate->encr_alg != SEOS_ENCRYPTION_AES ||
            candidate->hash_alg != SEOS_HASHING_SHA256) {
        NRF_LOG_ERROR("SEOS loadcb: invalid persisted credential data");
        return 0;
    }
    m_tag_information = candidate;
    ...
    nfc_tag_14a_handler_t handler = {
        ...
        .cb_activated = nfc_tag_seos_activate,
